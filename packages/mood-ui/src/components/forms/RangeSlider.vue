        ...
        >
          <span
            :class="[
              'block rounded-full',
              markSizeClass,
              mark.value >= fromValue && mark.value <= toValue
                ? fillColorClass
                : 'bg-muted-foreground',
            ]"
          />
...
function onTrackPointerDown(e: PointerEvent) {
  ...
  const thumb: Thumb =
    Math.abs(v - fromValue.value) <= Math.abs(v - toValue.value)
      ? "from"
      : "to";
...
const displayFrom = computed(() =>
  props.formatValue
    ? props.formatValue(fromValue.value)
