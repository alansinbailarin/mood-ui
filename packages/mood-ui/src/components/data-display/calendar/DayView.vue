<template> 
    ...
    > 
        ...
        <div v-if="showAllDayRow && allDayRows > 0" class="flex border-b border-border" :style="{ paddingRight: scrollbarWidth + 'px' }"> 
            <div class="w-14 shrink-0 flex items-start justify-end pr-2 pt-1 text-[10px] font-medium text-muted-foreground uppercase tracking-wider"> 
                all-day 
            </div> 
            <div 
                class="relative flex-1" 
                :style="[ 
                    { height: `${allDayRows * 22 + 4}px` }, 
                    dayIsDisabled ? disabledStripePattern : undefined, 
                ]" 
                @click="handleAllDayCellClick($event)" 
                @dblclick="handleAllDayCellDblClick($event)" 
                @dragover.prevent="onAllDayDragOver($event)" 
                @drop.prevent="onAllDayDrop($event)" 
                :class="[ 
                    'transition-colors', 
                    dayIsDisabled ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-muted/60', 
                    allDayDragOver ? 'bg-muted' : '', 
                ]" 
            > 
                <div 
        ...
        <div ref="scrollRef" class="flex-1 overflow-y-scroll relative" style="scrollbar-gutter: stable;"> 
            <div class="flex"> 
                <div class="w-14 shrink-0 relative" :style="{ height: `${totalPixels}px` }"> 
                    <div 
                ...
                <div 
                    ref="colRef" 
                    class="relative flex-1 border-l border-border" 
                    @pointerdown="onColPointerDown($event)" 
                    @click="handleSlotClick($event)" 
                    @dblclick="handleSlotDblClick($event)" 
                ...
                > 
                    <div 
                        ...
                        :class="[ 
                            ...
                            isSlotDisabled(idx) ? 'cursor-not-allowed' : '', 
                        ]" 
                    ></div> 
...
import type { DayView } from '../../../interfaces/data-display/calendar/DayView.interface'; 
...
const props = withDefaults(defineProps<DayView>(), { 
    modelValue: () => new Date(), 
    events: () => [], 
    locale: 'default', 
    color: 'default', 
...
const isSlotDisabled = (slotIdx: number) => { 
    if (dayIsDisabled.value) return true; 
    if (!props.disabledTimeRanges.length) return false; 
...
const rangeHitsDisabled = (startMinFromHourStart: number, endMinFromHourStart: number) => { 
    if (dayIsDisabled.value) return true; 
    if (!props.disabledTimeRanges.length) return false; 
...
const rangeOverlapsEvent = (startMs: number, endMs: number, ignoreId: CalendarEvent['id']) => { 
...
const timedSegments = computed<TimedSeg[]>(() => { 
    const result: TimedSeg[] = []; 
...
interface AllDaySeg { key: string; event: CalendarEvent; lane: number; } 
const allDayLayout = computed<{ segs: AllDaySeg[]; rows: number }>(() => { 
    ...
    const list = props.events 
        ...
        .sort((a, b) => a.start.getTime() - b.start.getTime()); 
    list.forEach((ev, i) => segs.push({ key: `ad-${ev.id}`, event: ev, lane: i })); 
    return { segs, rows: segs.length }; 
}); 
...
const handleSlotClick = (e: MouseEvent) => { 
    ...
    if (isSlotDisabled(slotIdx)) return; 
...
const handleSlotDblClick = (e: MouseEvent) => { 
...
const handleAllDayCellClick = (_e: MouseEvent) => {}; 
const handleAllDayCellDblClick = (e: MouseEvent) => { 
    if (props.readonly) return; 
    if (dayIsDisabled.value) return; 
    emit('create-event', startOfDay(currentDate.value), true, e); 
}; 
...
const allDayDragOver = ref(false); 
...
const onDragOver = (e: DragEvent) => { 
    ...
    if (!props.allowDropOnDisabled) { 
        if (dayIsDisabled.value) blocked = true; 
            ...
            if (rangeHitsDisabled(snappedMin, snappedMin + durationMin)) blocked = true; 
        } 
    } 
    if (e.dataTransfer) e.dataTransfer.dropEffect = blocked ? 'none' : 'move'; 
    dragOverActive.value = !blocked; 
    handleAutoScroll(e.clientY); 
}; 
const onDrop = (e: DragEvent) => { 
    if (!props.draggableEvents || draggingId.value === null) return; 
    if (!props.allowDropOnDisabled && dayIsDisabled.value) { onDragEnd(); return; } 
    const ev = props.events.find(x => x.id === draggingId.value); 
    ...
    const durationMin = duration / MS_MIN; 
    if (!props.allowDropOnDisabled && rangeHitsDisabled(snappedMin, snappedMin + durationMin)) { onDragEnd(); return; } 
    ...
    if (props.preventOverlap && rangeOverlapsEvent(newStart.getTime(), newEnd.getTime(), ev.id)) { onDragEnd(); return; } 
...
const onAllDayDragOver = (e: DragEvent) => { 
    if (!props.draggableEvents || draggingId.value === null) return; 
    const blocked = !props.allowDropOnDisabled && dayIsDisabled.value; 
    if (e.dataTransfer) e.dataTransfer.dropEffect = blocked ? 'none' : 'move'; 
    allDayDragOver.value = !blocked; 
}; 
const onAllDayDrop = (_e: DragEvent) => { 
    if (!props.draggableEvents || draggingId.value === null) return; 
    if (!props.allowDropOnDisabled && dayIsDisabled.value) { onDragEnd(); return; } 
    const ev = props.events.find(x => x.id === draggingId.value); 
    if (!ev) { onDragEnd(); return; } 
...
interface RangeSelect { startMin: number; endMin: number; topPx: number; heightPx: number; } 
...
const onColPointerDown = (e: PointerEvent) => { 
    ...
    const pxPerMin = props.slotHeight / props.slotMinutes; 
    rangeSelect.value = { 
        startMin, endMin: startMin + props.slotMinutes, 
...
interface ResizePreview { topPx: number; heightPx: number; } 
const resizingId = ref<string | number | null>(null); 
const resizingPreview = ref<ResizePreview | null>(null); 
let resizeStartState: { event: CalendarEvent; startMin: number; minEndMin: number } | null = null; 
...
const onResizeStart = (ev: CalendarEvent, e: PointerEvent) => { 
    ...
    resizeStartState = { event: ev, startMin, minEndMin }; 
    const pxPerMin = props.slotHeight / props.slotMinutes; 
...
const onResizeMove = (e: PointerEvent) => { 
    if (!resizeStartState || !resizingPreview.value || !colRef.value) return; 
    handleAutoScroll(e.clientY); 
    const rect = colRef.value.getBoundingClientRect(); 
...
const onResizeEnd = () => { 
    ...
    if (resizeStartState && resizingPreview.value) { 
        ...
        const hitsDisabled = !props.allowResizeOverDisabled && rangeHitsDisabled(resizeStartState.startMin, endMin); 
        const overlaps = props.preventOverlap && rangeOverlapsEvent(newStart.getTime(), newEnd.getTime(), resizeStartState.event.id); 
...
const handleKeydown = (e: KeyboardEvent) => { 
    ...
    else if (e.key.toLowerCase() === 'c' && !props.readonly) { e.preventDefault(); emit('create-event', startOfDay(currentDate.value), true, e); } 
}; 
