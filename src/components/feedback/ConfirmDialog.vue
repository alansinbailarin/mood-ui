...
watch(isOpen, async (open) => { 
    ...
    setTimeout(() => { 
        const el = (confirmBtnRef.value as unknown as { $el?: HTMLElement } | null)?.$el as 
            | HTMLElement 
            | undefined; 
        const btn = el?.tagName === 'BUTTON' ? (el as HTMLButtonElement) : el?.querySelector('button'); 
        btn?.focus(); 
    }, 60); 
}); 
...
const resolvedIcon = computed(() => { 
    ...
    switch (current.value?.color) { 
        ...
        case 'primary': return QuestionMarkCircleIcon; 
        case 'default': 
