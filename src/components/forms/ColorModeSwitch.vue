...
watch(() => props.modelValue, () => nextTick(() => measure(firstMeasureDone.value)));
watch(() => props.showLabels, () => nextTick(() => measure(false)));
watch(() => props.variant,    () => { isExpanded.value = false; nextTick(() => measure(false)); });
watch(isExpanded, () => nextTick(() => measure(false)));

const sz = {
    small:  { track: 'p-0.5', gap: 'gap-0.5', btn: 'w-7 h-7',   icon: 'w-3.5 h-3.5', lbl: 'text-xs px-2'   },
    medium: { track: 'p-0.5', gap: 'gap-0.5', btn: 'w-8 h-8',   icon: 'w-4 h-4',     lbl: 'text-sm px-2.5' },
    large:  { track: 'p-1',   gap: 'gap-1',   btn: 'w-10 h-10', icon: 'w-5 h-5',     lbl: 'text-sm px-3'   },
};
</script>
            ...
            >
                <button
                    ...
                    :class="[
                        ...
                        modelValue === opt.value ? 'text-foreground' : 'hover:text-foreground',
