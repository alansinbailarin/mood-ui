...
const {
    theme, color, radius, size, halo,
    primaryHex, accentHex, baseIntensity,
    darkSurface, localeName,
    reset: resetTheme,
    DEFAULTS,
...
const darkSurfaceComputed = computed(() => {
    ...
    const tinted = tintedDarkSurfaces(accentHex.value, baseIntensity.value / 100);
    return { ...base, ...tinted };
});
...
<template>
    ...
    >
        ...
        <div v-if="isWelcome" class="min-h-dvh bg-background text-foreground">
            <header class="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-background/70 bg-background/85 border-b border-border">
                <HeaderBar
                    :active-id="currentId"
                    :show-logo="true"
                    :theme="theme" :color="color" :radius="radius" :size="size" :halo="halo"
                    :primary-hex="primaryHex" :accent-hex="accentHex" :base-intensity="baseIntensity"
                    :dark-surface="darkSurface" :locale="localeName"
                    @navigate="onHeaderNavigate" @open-search="paletteOpen = true"
                    @update:theme="theme = $event" @update:color="color = $event"
                    @update:radius="radius = $event" @update:size="size = $event"
                    @update:halo="halo = $event" @update:primary-hex="primaryHex = $event"
                    @update:accent-hex="accentHex = $event" @update:base-intensity="baseIntensity = $event"
                    @update:dark-surface="darkSurface = $event" @update:locale="localeName = $event"
                    @reset="resetTheme"
                />
            </header>
            <div style="view-transition-name: showroom-content">
                <Suspense>
                    ...
                    <template #fallback>
                        <div class="flex items-center justify-center py-24"><Loader size="medium" /></div>
                    </template>
                </Suspense>
            </div>
        </div>
        ...
        ───────────────────────────────────────────────────────────── -->
        <div v-else-if="isThemeStudio" class="min-h-dvh bg-background text-foreground">
            <header class="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-background/70 bg-background/85 border-b border-border">
                <HeaderBar
                    :active-id="currentId"
                    :show-logo="true"
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8" style="view-transition-name: showroom-content">
                <Suspense>
                    <component :is="ActivePage" v-if="ActivePage" />
                    <template #fallback>
                        <div class="flex items-center justify-center py-24"><Loader size="medium" /></div>
